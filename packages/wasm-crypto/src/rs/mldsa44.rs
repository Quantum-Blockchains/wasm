...
fn new_from_seed(seed: &[u8]) -> ml_dsa_44::Keypair {
	ml_dsa_44::Keypair::generate(Some(seed))
}
...
pub fn ext_mldsa_sign(_: &[u8], seed: &[u8], message: &[u8]) -> Vec<u8> {
	...
	let signature = sk.sign(message, None, false);
...
pub fn ext_mldsa_verify(signature: &[u8], message: &[u8], pubkey: &[u8]) -> bool {
	let pk: ml_dsa_44::PublicKey = ml_dsa_44::PublicKey::from_bytes(pubkey);
	if signature.len() != 2420 {
		return false;
	}
	pk.verify(message, signature, None)
}
...
pub mod tests {
	...
	// 	let test = ml_dsa_44::Keypair::generate(Some(&seed));
	// 	let keypair = ext_mldsa_from_seed(&seed);
